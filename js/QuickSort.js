...
DESCRIPTION: QuickSort Visualisation
	...
	if(state == "select_pivot") {
		...
		setTimeout(function() {
			if(PARTITION_START == PARTITION_END) {
				...
				pivot = PARTITION_START;
				ARRAY_OF_OBJECTS[pivot].setPivot();
				updateActionTracker("The pivot is: ", ARRAY_OF_OBJECTS[pivot].number);
				displayBars();
				if(keepHistory()) {
					saveAnimationState();
					_textHistory.push("The pivot is: " + ARRAY_OF_OBJECTS[pivot].number);
				}
				if(pivot == 0) {
